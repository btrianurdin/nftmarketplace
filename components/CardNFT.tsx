...
import { INftAsset } from "../interface";
...
export default function CardNFT({ nftItem, title, listings }: CardNFTProps) {
  const [isListed, setIsListed] = useState(false);
  const [price, setPrice] = useState(0);
  const router = useRouter();

  useEffect(() => {
    const listing = listings && listings.find((listing: any) => listing.asset.id === nftItem.id);
    if (!!listing) {
      setIsListed(true);
    ...
    >
      ...
      <div className={style.details}>
        ...
        <div className={style.likes}>
          <span className={style.likeIcon}><HiHeart /></span>
          {' '}
          Likes
        </div>
      </div>
